...
import { isAllowedUser } from './GameEngine.js';
...
function disconnectDuplicateSession(username, currentSocketId) {
...
function sendPrivateState(room, socketId, playerId) {
...
io.on('connection', (socket) => {
    ...
    socket.on('join_room', (payload, ack) => {
        ...
        if (asSpectator) {
            joinUsername = payload?.displayName ?? `Spectator`;
        }
        ...
        const player = room.addPlayer(asSpectator ? joinUsername : joinUsername, socket.id, asSpectator);
